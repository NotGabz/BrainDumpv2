...
from utils.database import (
    ...
    get_setting, record_study_day, create_deck, get_decks, delete_deck, rename_deck,
)
from utils.ai import generate_flashcards
from utils.helpers import truncate_text, render_sidebar, inject_custom_css, require_login, play_correct_sound, play_wrong_sound, celebrate_confetti
...
# ── RESULTS MODE ────────────────────────────────────────────
    ...
    # Confetti if all rated Easy
    if all(d == "easy" for d in difficulties) and total > 0:
        celebrate_confetti("⭐", 4)
        st.balloons()
