...
if st.session_state.notes_view == "list":
    ...
    st.caption("Speak your notes and we'll transcribe them")
    voice_text = st.text_area("Paste transcribed text (or type your spoken notes)", height=120,
                              placeholder="Paste speech-to-text output here, or type spoken notes...",
                              key="voice_input")
    if voice_text.strip():
        vc1, vc2 = st.columns(2)
        with vc1:
            if st.button("🧹 Clean Up with AI", key="voice_cleanup"):
                from utils.ai import explain_concept
                cleaned, err = explain_concept(f"Clean up and organize these spoken notes into well-structured study notes with markdown formatting: {voice_text}")
                if not err:
                    st.session_state.voice_cleaned = cleaned
                    st.rerun()
        with vc2:
            if st.button("💾 Save as Note", type="primary", key="voice_save"):
                final = st.session_state.get("voice_cleaned", voice_text)
                title = "Voice Note"
                for line in final.split("\n"):
                    if line.startswith("# "):
                        title = line[2:].strip(); break
                nid = add_note(username, title, "Voice Note", final)
                if nid:
                    st.toast("Voice note saved!", icon="✅")
                    st.session_state.notes_view = "view"; st.session_state.selected_note = nid; st.rerun()
        if "voice_cleaned" in st.session_state:
            st.markdown("**AI Cleaned Version:**")
            st.markdown(st.session_state.voice_cleaned)

    # ── Image Upload (OCR) ──────────────────────────────────
    st.markdown("---")
    st.markdown("### 📸 Upload Image")
    st.caption("Upload a photo of handwritten notes to extract text")
    ...
    if img_file:
        ...
        if st.button("🔍 Read Image", key="ocr_read"):
            try:
        ...
        if "ocr_text" in st.session_state:
            st.text_area("Extracted text", value=st.session_state.ocr_text, height=150, key="ocr_display")
            oc1, oc2 = st.columns(2)
            with oc1:
                if st.button("🧹 Clean Up with AI", key="ocr_cleanup"):
                    from utils.ai import explain_concept
                    cleaned, err = explain_concept(
                        f"Clean up these OCR notes. Fix spelling, grammar, and organize into clear study notes with markdown: {st.session_state.ocr_text}"
                    )
                    if not err:
                        st.session_state.ocr_cleaned = cleaned
                        st.rerun()
            with oc2:
                if st.button("💾 Save as Note", type="primary", key="ocr_save"):
                    final = st.session_state.get("ocr_cleaned", st.session_state.ocr_text)
                    title = img_file.name.rsplit(".", 1)[0]
                    nid = add_note(username, title, "Image Notes", final)
                    if nid:
                        st.toast("Image note saved!", icon="✅")
                        st.session_state.notes_view = "view"; st.session_state.selected_note = nid; st.rerun()
            if "ocr_cleaned" in st.session_state:
                st.markdown("**AI Cleaned Version:**")
                st.markdown(st.session_state.ocr_cleaned)
